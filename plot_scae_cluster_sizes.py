...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot the distribution of node counts per cluster for the SCAE partition."
        )
    )
    parser.add_argument(
        "--partition-json",
        type=Path,
        default=Path("scae_partitions.json"),
        help="Partition JSON exported by train_rgcn_scae.py (defaults to scae_partitions.json).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("scae_out/scae_cluster_size_hist.png"),
...
def main() -> None:
    ...
    ax.set_title("RGCN-SCAE Cluster Size Distribution")
