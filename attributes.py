...
class FloatAttribute(neat.attributes.FloatAttribute):
    ...
    def init_value(self, config):
        ...
        if hasattr(config, self.min_value_name):
            self.min_value = max(
                getattr(config, self.min_value_name), (mean - (2 * stdev))
            )
        else:
            self.min_value = mean - (2 * stdev)
        if hasattr(config, self.max_value_name):
            self.max_value = min(
                getattr(config, self.max_value_name), (mean + (2 * stdev))
            )
        else:
        ...
        raise RuntimeError(
            "Unknown init_type {!r} for {!s}".format(
                getattr(config, self.init_type_name), self.init_type_name
            )
        )
...
class BoolAttribute(neat.attributes.BoolAttribute):
    ...
    def init_value(self, config):
        ...
        raise RuntimeError(
            "Unknown default value {!r} for {!s}".format(default, self.name)
        )
    ...
    def mutate_value(self, value, config):
        ...
        if mutate_rate > 0:
            if random() < mutate_rate:
                options = getattr(
                    config, self.options_name, getattr(self, "options", [])
                )
                if options:
                    return choice(options)
                return value
